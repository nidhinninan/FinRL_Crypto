...
def pct_to_log_return(pct_returns, fillna=True):
    if _is_pandas(pct_returns):
        ...
        if fillna:
          ...
          # Example modification idea for Log Returns
          value_to_log = 1 + pct_returns + 1e-8
          if isinstance(value_to_log, np.ndarray): # Handle array case
            # Set non-positive values to a small positive number before log
            value_to_log[value_to_log <= 0] = 1e-9
            return np.log(value_to_log)
          elif value_to_log > 0: # Handle scalar case
            return np.log(value_to_log)
          else:
            return -np.inf # Or some large negative number
"""
    else:
        if fillna:
            pct_returns = np.nan_to_num(pct_returns)
      ...
      vol = np.nanstd(excess, axis=0, ddof=1)
        ...
        if np.isscalar(excess_mean):
            ...
            if std_dev is None or np.isclose(std_dev, 0) or not np.isfinite(std_dev):
                ...
                # Calculate Sharpe normally for valid scalar std dev
                sharpe = np.sqrt(factor) * excess_mean / std_dev
        else:
            # Handle array case (multiple return streams, e.g., from PBO paths)
...
"""
        ...
        std_dev = np.nanstd(excess, axis=0, ddof=1)
        if std_dev is None or np.isclose(std_dev, 0) or not np.isfinite(std_dev):
          sharpe = 0.0 # Or maybe -1.0 or some other indicator of failure
        else:
          sharpe = np.sqrt(factor) * excess_mean / std_dev
